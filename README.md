...
### 3. Get an Instagram access token and your Instagram Business Account ID

This project authenticates to Instagram using Meta's **Instagram Business
Login** flow (not the older Facebook-Page-token route). It's done entirely
on Meta's own site, since it needs your login:

1. In the app dashboard, go to **Instagram → API setup with Instagram business login**, and follow Meta's steps to connect the YouthsToday Instagram account (it must be a **Business** or **Creator** account, not Personal).
2. Use Meta's token generator at `developers.facebook.com/instagram/token_generator/oauth/` to run through the OAuth consent screen and generate an access token for that Instagram account.
3. The generator's result screen shows both your **access token** (this is `META_IG_ACCESS_TOKEN`) and your **Instagram user ID** (this is `META_IG_USER_ID`) — copy both down immediately, since the screen doesn't stay open indefinitely.

This token is scoped only to Instagram content publishing and is separate
from the Facebook Page token in the next step — the two are not
interchangeable.

### 4. Get your Page ID and a long-lived Page access token (for Facebook posting)

Facebook posting uses a separate, traditional Page access token. In the app
dashboard, add the **Facebook Login for Business** or **Pages API** use
case ("Manage everything on your Page"), then get the token via Meta's
**Graph API Explorer**:

1. Go to [developers.facebook.com/tools/explorer](https://developers.facebook.com/tools/explorer), and in the top-right dropdowns select your app and your Facebook user.
2. Click **Generate Access Token** and grant: `pages_show_list`, `pages_read_engagement`, `pages_manage_posts`. This gives a **short-lived** user token (~1 hour) — expected, the next steps trade it up.
3. With that token selected, query `GET /me/accounts`. The response lists your Pages; find YouthsToday's entry and note its `id` (this is your **Page ID**) — ignore the `access_token` shown here, it's short-lived too.
4. Exchange the short-lived user token for a long-lived one:
   `https://graph.facebook.com/v21.0/oauth/access_token?grant_type=fb_exchange_token&client_id=<APP_ID>&client_secret=<APP_SECRET>&fb_exchange_token=<SHORT_LIVED_USER_TOKEN>`
   (App ID and App Secret are on your app's **Settings → Basic** page.) This returns a long-lived **user** token, valid about 60 days.
5. Query `GET /me/accounts` **again**, using that long-lived user token as `access_token`. The `access_token` field for YouthsToday's Page in this response is your long-lived **Page** access token (`META_PAGE_ACCESS_TOKEN`) — the one the script actually uses for Facebook. Per Meta's documentation, a Page token obtained this way has no fixed expiration; it only stops working if the Page's permissions are revoked, your password changes, or it goes unused for an extended period.

Steps 1–5 above are Meta's process, done entirely on Meta's own site —
nothing here automates it.
...
secret**. Add these five (paste each value directly into GitHub's form —
never into a chat with Claude or anyone else):

| Secret name | Value |
|---|---|
| `META_IG_ACCESS_TOKEN` | the Instagram access token from step 3.3 |
| `META_PAGE_ACCESS_TOKEN` | the long-lived Page token from step 4.5 |
| `META_PAGE_ID` | the Page ID from step 4.3 |
| `META_IG_USER_ID` | the Instagram user ID from step 3.3 |
...
The Instagram token from step 3 and the long-lived Page token from step 4.5
can each expire or be revoked independently — Meta can invalidate either one
if you change your password, remove the app's permissions, or don't use it
for a long stretch. If a scheduled run ever fails with an authorization
error, check which platform failed in the run's log: an Instagram failure
means redoing step 3 (generating a fresh `META_IG_ACCESS_TOKEN`); a Facebook
failure means redoing steps 4.4–4.5 (a fresh `META_PAGE_ACCESS_TOKEN`).
Update just that one secret — everything else stays the same. GitHub
Actions will show a red X on the run and, if you have notifications on for
the repo, email you when that happens.

## Manual runs and backfilling

- Post a specific date on demand: **Actions → Run workflow**, fill in the `date` input.
- Run locally for debugging: `IMAGE_BASE_URL=... META_IG_ACCESS_TOKEN=... META_PAGE_ACCESS_TOKEN=... META_PAGE_ID=... META_IG_USER_ID=... node scripts/post.js --dry-run`
